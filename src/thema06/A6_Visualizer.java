...
public class A6_Visualizer {
	...
	public static void main(String[] args) {
		double[] Messwerte = new double[util.MessstationDummy.getSENSOR_TYPES().length];
		for (int i = 0; i < Messwerte.length; i++) {
			Messwerte[i] = Double.NaN; // mit NaN initialisieren um ungültige Messwerte sichtbar zu machen
		}
		...
		// Ausgabe Messwerte
		for (int i = 0; i < Messwerte.length; i++) {
			System.out.print(String.format("%6s", Messwerte[i]) + " " + Messeinheiten[i] + " \t");
		}
		System.out.println(""); // neue Zeile
	}
